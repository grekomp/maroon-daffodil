      ...
      <React.Fragment>
        {_.get(field, "input_type", null) !== "checkbox" &&
          _.get(field, "label", null) && (
            <label
              id={_.get(field, "name", null) + "-label"}
              htmlFor={_.get(field, "name", null)}
            >
              {_.get(field, "label", null)}
            </label>
          )}
        {_.get(field, "input_type", null) === "checkbox" ? (
          <div className="form-checkbox">
            <input
              ...
              {...(_.get(field, "label", null)
                ? { "aria-labelledby": _.get(field, "name", null) + "-label" }
                : null)}
              {...(_.get(field, "is_required", null)
                ? { required: true }
                : null)}
            />
            {_.get(field, "label", null) && (
              <label
                ...
                id={_.get(field, "name", null) + "-label"}
              >
                {markdownify(_.get(field, "label", null))}
              </label>
            )}
          </div>
