        ...
        {_.map(categories, (category, category_idx) => {
          ...
          return category_data.link ? (
            <React.Fragment key={category_idx + ".1"}>
              <Link key={category_idx} href={withPrefix(category_data.link)}>
                {category_data.title}
              </Link>
              {!(category_idx === category_len - 1) && ", "}
            </React.Fragment>
          ) : (
            <React.Fragment key={category_idx + ".3"}>
              <span key={category_idx + ".2"}>{category_data.title}</span>
              {!(category_idx === category_len - 1) && ", "}
            </React.Fragment>
          );
        })}
