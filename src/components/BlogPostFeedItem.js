...
export default class BlogPostFeedItem extends React.Component {
  render() {
    ...
    if (_.get(post, "frontmatter.thumb_image", null)) {
      has_image = true;
    }
    ...
    if (
      _.get(blog_feed_section, "show_categories", null) &&
            ...
            <div className="flex flex-column">
              ...
              <div
                className={classNames("item__body", {
                  "px-3": is_card,
                  "pb-3": is_card,
                  "pt-3": is_card && no_image,
                  "px-sm-4": is_card,
                  "pb-sm-4": is_card,
                  "pt-4": is_card && no_image,
                })}
              >
                ...
                {_.get(blog_feed_section, "show_excerpt", null) &&
                  _.get(post, "frontmatter.excerpt", null) && (
                    <div className="item__copy">
                      <p>{_.get(post, "frontmatter.excerpt", null)}</p>
                    </div>
                  )}
                {_.get(blog_feed_section, "show_author", null) &&
                  _.get(post, "frontmatter.author", null) && (
                    <BlogPostAuthor
                      {...this.props}
                      author={_.get(post, "frontmatter.author", null)}
                      container_class={"item__byline"}
                      avatar_size={"small"}
                    />
                  )}
              </div>
