...
export default class BlogPostAuthor extends React.Component {
  render() {
    ...
    let author_data = getData(this.props.data, author);
    return (
      <div className={container_class}>
        {author_data.link ? (
          <Link
            className="flex items-center"
            href={withPrefix(author_data.link)}
          >
            {author_data.photo && (
              <figure
                className={classNames("avatar", "mr-1", {
                  "avatar--small": avatar_size === "small",
                })}
              >
                <img
                  className="avatar__img"
                  src={withPrefix(author_data.photo)}
                  alt={author_data.photo_alt}
                />
              </figure>
            )}
            <span>
